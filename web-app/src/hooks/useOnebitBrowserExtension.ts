...
export function useJanBrowserExtension() {
  ...
  const handleConnectionSuccess = useCallback(() => {
    ...
    toast.success('onebit browser tools enabled')
  }, [])
  ...
  const toggleBrowser = useCallback(async () => {
    // Synchronous check — refs update synchronously, prevents race conditions
    ...
    try {
      ...
      if (newActiveState) {
        ...
        toast.success('onebit browser tools disabled')
